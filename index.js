...
client.once('ready', async () => {
    ...
    client.user.setActivity(`${servers} server(s) and ${users} user(s).`, {
        type: 'LISTENING',
    });
});

client.on('guildMemberAdd', guildMember => {
    const channel = guildMember.guild.channels.cache.find(ch => ch.name === 'welcome-and-boosts');
    if (!channel) return;
    channel.send(`Welcome to the server, ${guildMember}!`);
    guildMember.roles.add(guildMember.guild.roles.cache.find(role => role.name === "floppalet"));
});
